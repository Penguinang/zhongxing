...
public class Planet : MonoBehaviour {
    ...
    private void FixedUpdate()
    {
        ...
        Gforce = new Vector2(this.transform.position.x- ship.transform.position.x ,
            this.transform.position.y- ship.transform.position.y);
        float R = (this.transform.position - ship.transform.position).magnitude;
        Gforce = Gforce * mass/R*R;
        Rship = ship.GetComponent<Ship>().Rship;

        Rship.AddForce(Gforce);
